            ...
            for batch_dict in self.train_dataloader:
                ...
                if hasattr(self, 'curriculum_sampler') and self.curriculum_sampler is not None:
                    ...
                    for source, stats in source_stats.items():
                        metrics[f'curriculum/expected_value/{source}'] = stats['expected_value']
                        metrics[f'curriculum/reward_rate/{source}'] = stats['reward_rate']
                        metrics[f'curriculum/sampling_weight/{source}'] = stats.get('sampling_weight', 0.0)
                        metrics[f'curriculum/sample_count/{source}'] = stats['n_samples']
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if hasattr(self, 'curriculum_sampler') and self.curriculum_sampler is not None:
                            ...
                            for i, source in enumerate(current_data_sources):
                                ...
                                sample_rewards = rewards[i].sum().item()
                                # Use response_mask to focus on response tokens if available
                                if 'responses' in batch.batch and 'attention_mask' in batch.batch:
                                    ...
                                    sample_advantages = advantages[i].mean().item()
                                
                                data_source_rewards[source].append(sample_rewards)
                                data_source_advantages[source].append(sample_advantages)
                            
                            # Update curriculum sampler with training batch data
